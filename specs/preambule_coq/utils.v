...
unfold blt_nat. 
intuition auto with *. inversion H. inversion H. intuition auto with *. intuition auto with *. unfold blt_nat in H. discriminate H. 
inversion H. intuition.  rewrite <- Nat.succ_lt_mono. apply IHx. unfold blt_nat in H. apply H. apply Nat.succ_lt_mono in H.
apply IHx in H. 
...
Proof.
intros x H; apply Nat.lt_irrefl with x; apply -> blt_lt; trivial.
Qed.

Theorem blt_nat_trans : forall x y z,
  blt_nat x y = true -> blt_nat y z = true -> blt_nat x z = true.
Proof.
intros x y z Hxy Hyz; apply <- blt_lt; apply Nat.lt_trans with y; apply -> blt_lt; trivial.
Qed.
