...
Require Import erl_insin_spec.



Definition type_LF_155 :=  PLAN ->  nat ->  nat ->  nat ->  nat -> (Prop * (List.list term)).

Definition F_155 : type_LF_155:= (fun   u1 u2 u3 _ _ => ((erl (insIn u1 u2 u3)) = u3, (Term id_erl ((Term id_insIn ((model_PLAN u1):: (model_nat u2):: (model_nat u3)::nil))::nil))::(model_nat u3)::nil)).
...
Definition F_191 : type_LF_155:= (fun    _ u3 _ _ _ => (u3 = u3, (model_nat u3)::(model_nat u3)::nil)).
Definition F_185 : type_LF_155:= (fun   u6 u3 u8 u9 _ => ((le u9 u3) = true -> (erl (insIn u6 (time (C u8 u9)) u3)) = u3, (Term id_le ((model_nat u9):: (model_nat u3)::nil))::(Term id_true nil)::(Term id_erl ((Term id_insIn ((model_PLAN u6):: (Term id_time ((Term id_C ((model_nat u8):: (model_nat u9)::nil))::nil)):: (model_nat u3)::nil))::nil))::(model_nat u3)::nil)).
Definition F_188 : type_LF_155:= (fun    _ u2 u3 u8 u9 => ((le (er (C u8 u9)) u3) = false -> (er (C u2 u3)) = u3, (Term id_le ((Term id_er ((Term id_C ((model_nat u8):: (model_nat u9)::nil))::nil)):: (model_nat u3)::nil))::(Term id_false nil)::(Term id_er ((Term id_C ((model_nat u2):: (model_nat u3)::nil))::nil))::(model_nat u3)::nil)).
Definition F_197 : type_LF_155:= (fun    _ u3 u8 u9 _ => ((le (er (C u8 u9)) u3) = false -> u3 = u3, (Term id_le ((Term id_er ((Term id_C ((model_nat u8):: (model_nat u9)::nil))::nil)):: (model_nat u3)::nil))::(Term id_false nil)::(model_nat u3)::(model_nat u3)::nil)).

Definition LF_155 := [F_155, F_167, F_173, F_179, F_182, F_191, F_185, F_188, F_197].
...
assert (HFabs0 : fst (F_173 u6 _u3 u8 u9 0)).
...
assert (HFabs1 : fst (F_185 u6 u3 u8 u9 0)).
...
assert (HFabs1 : fst (F_188 Nil u2 u3 u8 u9)).
apply Res. trivial_in 7. unfold snd. unfold F_188. unfold F_179. rewrite_model. abstract solve_rpo_mul.
unfold fst. unfold F_179. unfold fst in HFabs1. unfold F_188 in HFabs1.   
...
assert (HFabs1 : fst (F_191 Nil u3 0 0 0)).
apply Res. trivial_in 5. unfold snd. unfold F_191. unfold F_182. rewrite_model. abstract solve_rpo_mul.
unfold fst. unfold F_182. unfold fst in HFabs1. unfold F_191 in HFabs1.   
...
assert (HFabs1 : fst (F_155 u6 u8 u3 0 0)).
apply Res. trivial_in 0. unfold snd. unfold F_155. unfold F_185. rewrite_model. abstract solve_rpo_mul.
unfold fst. unfold F_185. unfold fst in HFabs1. unfold F_155 in HFabs1.   
...
assert (HFabs1 : fst (F_197 Nil u3 u8 u9 0)).
apply Res. trivial_in 8. unfold snd. unfold F_197. unfold F_188. rewrite_model. abstract solve_rpo_mul.
unfold fst. unfold F_188. unfold fst in HFabs1. unfold F_197 in HFabs1.   
pattern u2, u3. simpl (er _). cbv beta.
 simpl. auto.
...
Theorem true_155: forall (u1: PLAN) (u2: nat) (u3: nat), (erl (insIn u1 u2 u3)) = u3.
Proof.
do 3 intro.
