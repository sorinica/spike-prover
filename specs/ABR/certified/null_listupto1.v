...
Require Import null_listupto1_spec.



Definition type_LF_155 :=  PLAN ->  nat ->  nat ->  nat -> (Prop * (List.list term)).

Definition F_155 : type_LF_155:= (fun   u1 u2 _ _ => ((listUpTo u1 u2) = Nil -> u1 = Nil, (Term id_listUpTo ((model_PLAN u1):: (model_nat u2)::nil))::(Term id_Nil nil)::(model_PLAN u1)::(Term id_Nil nil)::nil)).
...
Definition F_173 : type_LF_155:= (fun   u6 u2 u3 u5 => ((Cons (C u3 u5) Nil) = Nil -> (le u3 u2) = true -> (Cons (C u3 u5) u6) = Nil, (Term id_Cons ((Term id_C ((model_nat u3):: (model_nat u5)::nil)):: (Term id_Nil nil)::nil))::(Term id_Nil nil)::(Term id_le ((model_nat u3):: (model_nat u2)::nil))::(Term id_true nil)::(Term id_Cons ((Term id_C ((model_nat u3):: (model_nat u5)::nil)):: (model_PLAN u6)::nil))::(Term id_Nil nil)::nil)).
Definition F_179 : type_LF_155:= (fun   u6 u2 u3 u5 => ((Cons (C u3 u5) (listUpTo u6 u2)) = Nil -> (le u3 u2) = false -> (Cons (C u3 u5) u6) = Nil, (Term id_Cons ((Term id_C ((model_nat u3):: (model_nat u5)::nil)):: (Term id_listUpTo ((model_PLAN u6):: (model_nat u2)::nil))::nil))::(Term id_Nil nil)::(Term id_le ((model_nat u3):: (model_nat u2)::nil))::(Term id_false nil)::(Term id_Cons ((Term id_C ((model_nat u3):: (model_nat u5)::nil)):: (model_PLAN u6)::nil))::(Term id_Nil nil)::nil)).
...
Theorem true_155: forall (u1: PLAN) (u2: nat), (listUpTo u1 u2) = Nil -> u1 = Nil.
Proof.
do 2 intro.
