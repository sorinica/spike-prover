...
Require Import sorted_e_two_spec.



Definition type_LF_155 :=  PLAN ->  OBJ ->  nat ->  nat ->  nat -> (Prop * (List.list term)).

Definition F_155 : type_LF_155:= (fun  u2 u1  _ _ _ => ((sortedE (Cons u1 u2)) = true -> (sortedE u2) = true, (Term id_sortedE ((Term id_Cons ((model_OBJ u1):: (model_PLAN u2)::nil))::nil))::(Term id_true nil)::(Term id_sortedE ((model_PLAN u2)::nil))::(Term id_true nil)::nil)).
Definition F_180 : type_LF_155:= (fun  u7  _ u3 u4 u6 => ((sortedE (Cons (C u6 u3) u7)) = true -> (le u3 u4) = false -> (sortedE (Cons (C u6 u3) u7)) = true, (Term id_sortedE ((Term id_Cons ((Term id_C ((model_nat u6):: (model_nat u3)::nil)):: (model_PLAN u7)::nil))::nil))::(Term id_true nil)::(Term id_le ((model_nat u3):: (model_nat u4)::nil))::(Term id_false nil)::(Term id_sortedE ((Term id_Cons ((Term id_C ((model_nat u6):: (model_nat u3)::nil)):: (model_PLAN u7)::nil))::nil))::(Term id_true nil)::nil)).
Definition F_174 : type_LF_155:= (fun  u7  _ u3 u4 u6 => (false = true -> (le u3 u4) = true -> (sortedE (Cons (C u6 u3) u7)) = true, (Term id_false nil)::(Term id_true nil)::(Term id_le ((model_nat u3):: (model_nat u4)::nil))::(Term id_true nil)::(Term id_sortedE ((Term id_Cons ((Term id_C ((model_nat u6):: (model_nat u3)::nil)):: (model_PLAN u7)::nil))::nil))::(Term id_true nil)::nil)).
Definition F_168 : type_LF_155:= (fun   _  _  _ _ _ => (true = true -> (sortedE Nil) = true, (Term id_true nil)::(Term id_true nil)::(Term id_sortedE ((Term id_Nil nil)::nil))::(Term id_true nil)::nil)).
Definition F_181 : type_LF_155:= (fun   _  _  _ _ _ => ((sortedE Nil) = true, (Term id_sortedE ((Term id_Nil nil)::nil))::(Term id_true nil)::nil)).
Definition F_184 : type_LF_155:= (fun   _  _  _ _ _ => (true = true, (Term id_true nil)::(Term id_true nil)::nil)).

Definition LF_155 := [F_155, F_180, F_174, F_168, F_181, F_184].


Function f_155 (u2: PLAN) (u1: OBJ) {struct u2} : bool :=
 match u2, u1 with
| Nil, _ => true
| (Cons (C u6 u3) u7), (C u5 u4) => true
end.
...
intros _u2 _u1. intro u6. intro u3. intro u7.  intro eq_1. intro u5. intro u4.  intro eq_2.  intro HFabs0.
...
apply Hind. trivial_in 2. unfold snd. unfold F_174. unfold F_155. rewrite_model. abstract solve_rpo_mul.
...
apply Hind. trivial_in 1. unfold snd. unfold F_180. unfold F_155. rewrite_model. abstract solve_rpo_mul.
...
Theorem true_155: forall (u2: PLAN) (u1: OBJ), (sortedE (Cons u1 u2)) = true -> (sortedE u2) = true.
Proof.
do 2 intro.
