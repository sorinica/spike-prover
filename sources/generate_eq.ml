...
let generate_eq verbose   dummy_cxt cxt (c:Clauses.peano_context Clauses.clause) is_strict =
...
(* 	let () = print_history_ic normalize c in  *)
  ...
  let fn5 (s, r_orig) = 
	...
	let pos, theta = lhs_norm#subterm_matching (fun p s -> s) lhs_orig in
