...
and process_monom (c,m) =  
  match m#content with
      Term (symb,l,_)  when symb = (id_symbol_plus) -> 
	let l_sorted = Sort.list (fun x y -> heavier y x) l in 
	let lc = List.map (fun x -> (c, x)) l_sorted in 
	normalize_monom_list lc 

    | Term (symb,_,_)  when symb = (id_symbol_zero) -> 0,[]

    | Term (symb,l,_)  when symb = (id_symbol_minus) -> 
    ...
    | Term (symb,l,_)  when symb = (id_symbol_s) ->
	assert (List.length l = 1);
    ...
    | Term (symb,l,_)  when !int_specif_defined && symb = (id_symbol_p) ->
	assert (List.length l = 1);
    ...
    | Term (symb,l,_) when symb = (id_symbol_times)  ->
	let test_zero t = t#syntactic_equal (term_nat 0) in
	if List.exists test_zero l
	then 0, [0, m]
	else 
	  let fn t p =
	  ...
	  if List.length true_pi = 1 
	  then (c * const_pi, (multiply_const (c,true_pi)))
	  else 
	    let c_norm, normalized_pi = normalize_monom_list true_pi in
	    let returned_const = c * (const_pi + c_norm) in
	    let returned_poly = multiply_const (c, normalized_pi)
	    in returned_const, returned_poly
