...
Module R_peano_deep_rew.
 ...
 Definition R_peano_rule_as_list  := R_peano_rule_as_list_1.
...
Module M := algebra.EQT.T.
Module Peano_rpo := rpo.Make(M).
...
Definition prec_bool (f1 f2:algebra.F.symb) : bool := 
    match f1,f2 with
      | algebra.F.id_0,algebra.F.id_0 => false
      | algebra.F.id_0,algebra.F.id__plus_ => true
      | algebra.F.id_0,algebra.F.id_s => true
      | algebra.F.id__plus_,algebra.F.id_0 => false
      | algebra.F.id__plus_,algebra.F.id__plus_ => false
      | algebra.F.id__plus_,algebra.F.id_s => false
      | algebra.F.id_s,algebra.F.id_0 => false
      | algebra.F.id_s,algebra.F.id__plus_ => true
      | algebra.F.id_s,algebra.F.id_s => false
      end.
...
Definition prec (f1 f2: algebra.F.symb) := lt (prec' f1) (prec' f2).
...
Lemma prec_transitive : transitive algebra.F.symb prec.
Proof.
unfold transitive.
intros x y z.
elim x.
...
Definition rpo_infos := (Peano_rpo.empty_rpo_infos precedence 3).

Definition rpo t1 t2 :=  Peano_rpo.rpo_eval rpo_infos 9 t1 t2.

Definition rpo_mult l1 l2 :=
                                match Peano_rpo.remove_equiv_eval_list (Peano_rpo.equiv_eval rpo_infos 9) l1 l2 with
                                | None => None
                                | Some (nil, nil) => Some ordered_set.Equivalent
                                | Some (nil, _ :: _) => Some ordered_set.Less_than
                                | Some (_ :: _,nil) => Some ordered_set.Greater_than
                                | Some (l1, l2) => Peano_rpo.mult_eval (Peano_rpo.rpo_eval rpo_infos 9) l1 l2
                                end.
...
Definition _x :=  (algebra.Alg.Var 0).

Definition _0 := (algebra.Alg.Term algebra.F.id_0 nil).

Definition s_x := (algebra.Alg.Term algebra.F.id_s (_x::nil)).

Definition plus_s_x_0 := (algebra.Alg.Term algebra.F.id__plus_ (s_x::_0::nil)).

Definition plus_x_0 := (algebra.Alg.Term algebra.F.id__plus_ (_x::_0::nil)).

Definition s_plus_x_0 := (algebra.Alg.Term algebra.F.id_s (plus_x_0::nil)).

(* less1 *)

Eval compute in rpo_mult (s_plus_x_0::s_x::nil) (plus_s_x_0::s_x::nil). 

(* less2 *)

Eval compute in rpo_mult (plus_x_0::_x::nil) (s_plus_x_0::s_x::nil).

(* other examples *)

Eval compute in rpo _0 s_x. 

Eval compute in rpo _x s_x.     
     
Eval compute in rpo s_plus_x_0 plus_s_x_0. 
 
Eval compute in rpo_mult ((algebra.Alg.Var 1)::nil) ((algebra.Alg.Var 1)::s_x::nil).
