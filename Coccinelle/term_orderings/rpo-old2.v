...
Proof.
intros rpo_infos A B H.
unfold rpo_mult_eval in H.
case_eq (remove_equiv_eval_list (equiv_eval rpo_infos rpo_infos.(bb)) A B).
intros (l1,l2) Hrem.
rewrite Hrem in H.
...
clear H.
assert (Hequiv:=remove_equiv_eval_list_is_sound (equiv_eval rpo_infos rpo_infos.(bb)) A B).
rewrite Hrem in Hequiv.
elim Hequiv; clear Hequiv.
intros ll (H1,(H2,(H3,_))).
apply (@List_mul _ t l nil A); trivial.
reflexivity.
...
intros.
apply rpo_mul_antirefl with rpo_infos []. trivial.
intros.
inversion H0.
intros.
inversion H1.
...
discriminate H8.
intros.
assert (forall (a : term) (l : list term) (rpo_infos : rpo_inf),
       permut0 equiv (a :: l) l0 ->
       rpo_mul (bb rpo_infos) (a :: l) l0 -> False).
apply H0; trivial. clear H0 H1.
inversion H3.
assert (rpo_mul (bb rpo_infos) (a1 :: l3) (a1 :: l3)-> False).
apply rpo_mul_antirefl.
apply H8.
apply List_mul with a2 lg ls lc. trivial.
apply permut0_trans with (a0:: l0). apply equiv_equiv. trivial. trivial.
trivial.
Qed.
