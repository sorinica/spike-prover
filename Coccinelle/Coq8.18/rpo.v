...
generalize ( Plus.plus_le_compat_r_stt _ _ (list_size size l) (size_ge_one a));
rewrite <- s_eq_t; trivial. 
do 2 rewrite size_unfold;
simpl; apply Arith_prebase.lt_n_S_stt; apply Nat.lt_le_trans with (1 + list_size size l);
auto with arith; 
apply Plus.plus_le_compat_r_stt; apply size_ge_one. 
Defined.
...
unfold o_size2, size2, lex;
generalize (beq_nat_ok (size s) (size s)); case (Nat.eqb (size s) (size s)); [intros _ | intro s_diff_s].
do 2 rewrite size_unfold; apply Plus.plus_lt_compat_l_stt; simpl.
exact (Plus.plus_le_compat_r_stt _ _ (list_size size l) (size_ge_one a)).
apply False_rec; apply s_diff_s; reflexivity.
Defined.
...
do 2 rewrite size_unfold.
simpl; apply le_n_S.
apply (Plus.plus_le_compat_r_stt 0 (size t1) (list_size size l1)).
...
apply False_ind; exact (Arith_prebase.gt_irrefl_stt 0 p_lt_k).
exact (refl_equal ).
...
apply Nat.le_trans with (list_size size (a1 :: k1)); [apply Arith_prebase.le_plus_r_stt | exact p_le_k].
...
refine (Nat.le_trans _ _ _ _ L); apply Arith_prebase.le_plus_r_stt.
...
apply Arith_prebase.lt_n_Sm_le_stt.
...
[apply Nat.le_add_r | apply Plus.plus_le_compat_r_stt; apply size_ge_one].
(* induction step *)
...
apply (Plus.plus_le_compat_r_stt 1 (size t) (list_size size l));
apply size_ge_one.
...
rewrite <- EqNat.beq_nat_refl_stt; simpl.
...
left; apply sym_eq; apply EqNat.beq_nat_true_stt; assumption.
...
apply False_rec; generalize (EqNat.beq_nat_true_stt _ _ L); clear L; induction ll as [ | [u1 u2] ll].
discriminate.
...
left; apply EqNat.beq_nat_true_stt; assumption.
...
apply Plus.plus_le_compat_r_stt; apply size_direct_subterm; trivial.
intros t2 t2_in_l2; apply IHn.
...
apply Plus.plus_le_compat_l_stt; apply size_direct_subterm; trivial.
intros t1 t2 t1_in_l1 t2_in_l2; apply IHn.
apply le_S_n; refine (Nat.le_trans _ _ _ _ St);
replace (S (size t1 + size t2)) with  (S (size t1) + size t2); trivial;
...
rewrite Nat.add_comm; apply Plus.plus_le_compat_l_stt.
apply size_direct_subterm; trivial.
...
simpl in T; absurd (@None comp = None); trivial.
destruct L as [L | [L1 L2]].
rewrite L; rewrite <- EqNat.beq_nat_refl_stt; intro; discriminate.
