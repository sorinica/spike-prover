...
Module Multiset (MC : MultisetCore).
  ...
  Section NewOperations.
    ...
    Definition member x M := mult x M > 0.
  ...
  Section MultisetLemmas.
    ...
    Lemma mult_insert M a : mult a (M + {{a}}) > 0.

    Proof.
      replace (a/(M + {{a}})) with ((mult a M) + (mult a {{a}}))%nat.
      replace (a/{{a}}) with 1.
      omega.
    ...
    Proof.
      unfold member. assert (a/{{a}} = 1); [idtac | omega].
    ...
    Proof.
      ...
      replace (a/(M+N)) with (mult a M + (mult a N))%nat.
      omega.
      auto with multisets.
    Qed.
    ...
    Proof. unfold member. rewrite (union_mult M N). omega. Qed.
    ...
    Proof.
      mset_unfold; intros.
      assert ((mult a M + (mult a N))%nat = (mult a M' + (mult a N'))%nat).
      rewrite <- !union_mult.
      apply meq_multeq; trivial.
      omega.
    Qed.
    ...
    Proof.
      ...
      assert ((mult x M + (mult x P))%nat = (mult x N + (mult x P))%nat).
      rewrite <- !union_mult.
      apply meq_multeq; trivial.
      omega.
    Qed.
    ...
    Proof.
      intros.
      try_solve_meq.
      assert ((mult x M + (mult x {{a}}))%nat = (mult x M' + (mult x {{a'}}))%nat).
      rewrite <- !union_mult.
      apply meq_multeq; trivial.
      omega.
    Qed.
    ...
    Proof.
      intros.
      assert ((mult a M + 1)%nat = (mult a M' + 0)%nat).
    ...
    Proof.
      ...
      assert ((mult x M + (mult x {{a}}))%nat = (mult x N + (mult x P))%nat).
      rewrite <- !union_mult.
      apply meq_multeq; trivial.
      case (eqA_dec x a); intro x_a.
      (* x =A= a *)
      ...
      assert ((mult x M + 1)%nat = (mult x N + (mult x P))%nat).
      rewrite <- (singleton_mult_in x_a); trivial.
      omega.
      trivial.
      (* ~ x =A= a *)
      rewrite (singleton_mult_notin x_a).
      assert ((mult x M + 0)%nat = (mult x N + (mult x P))%nat).
    ...
    Proof.
      intros; try_solve_meq.
      assert ((mult x M + (mult x N))%nat = x/empty).
  ...
  Section MultisetLemmas2.
    ...
    Proof.
      intros; try_solve_meq.
      assert ((mult x L + (mult x R))%nat = (mult x U + (mult x D))%nat).
      rewrite <- (union_mult L R), <- (union_mult U D).
      auto with multisets.
      case (Compare_dec.le_lt_dec (mult x L) (mult x U)); intro l_u.
      (* u >= l *)
      assert (mult x R >= mult x D). omega. rewrite (Min.min_r (mult x R) (mult x D)); omega.
      (* l > u *)
      assert (mult x R < mult x D). omega.
      rewrite Minus.not_le_minus_0, (Min.min_l (mult x R) (mult x D)); omega.
    Qed.
  ...
  Section List2Multiset.
    ...
    Proof.
      ...
      rewrite union_mult.
      assert (mult x (list2multiset l) > 0).
      apply (IHl x); trivial.
      omega.
    Qed.
    ...
    Proof.
      ...
      destruct (length m); simpl in *; omega.
    Qed.
  ...
  Section Multiset2List. (* William Delobel *)
    ...
    Definition multiset2list M := proj1_sig (mult_to_list M).
    ...
    Proof.
      ...
      gen (meq_sym Hx); intro Hx'.
      gen (union_isempty Hx'); intro Hx''.
  ...
  Section Multiset2List_Results.
    ...
    Proof.
      ...
      apply member_union_r; auto with multisets.  simpl. intros.
      discriminate. 
    Qed.
