...
Lemma rpo_mul_subst : forall A B: (list term), forall bb:nat,  rpo_mul bb A B -> 
...
trivial.
generalize (prec_eval_is_sound f1 f2); destruct (prec_eval f1 f2).
intro; subst f1; apply False_rec; apply (prec_antisym Prec  f2); trivial.
...
intro; subst f2; apply False_rec; apply (prec_antisym Prec f1); trivial.
intro f2_lt_f1; apply False_rec; apply (prec_antisym Prec f1); apply prec_transitive with f2; trivial.
intros _; assert (H' : list_forall_option
    (fun t : term =>
     match rpo_eval rpo_infos n (Term f2 l2) t with
...
intro; apply False_rec; apply (prec_antisym Prec f2); assumption.
intro; apply False_rec; apply (prec_antisym Prec f2); assumption.
...
Definition rpo_mult_eval rpo_infos n l1 l2 :=
                                ...
                                | Some (nil, _ :: _) => Some Less_than
                                | Some (_ :: _,nil) => Some Greater_than
                                | Some (l1, l2) => mult_eval (rpo_eval rpo_infos n) l1 l2
                                end.
...
(* Lemma rpo_mul_dickson1:  forall bb l l',  clos_trans (list term) (rpo_mul_step_rest bb) l l' -> rpo_mul_rest bb l l'. *)
(* Proof. *)
...
(* Lemma rpo_mul_remove_equiv_lists : *)
(*   forall bb l l' l1 l2,  permut0 equiv l1 l2 -> rpo_mul bb (l ++ l1) (l' ++ l2) -> rpo_mul bb l l'. *)
(* Proof. *)
(* double induction l l'. *)
(* simpl. intros. *)
(* Focus 2. *)
(* intros. *)
(* apply List_mul with a l0 nil nil. *)
(* simpl. apply list_permut.permut0_refl. apply equiv_equiv. *)
(* simpl. rewrite <- app_nil_end. *)
(* apply list_permut.permut0_refl. apply equiv_equiv. *)
(* simpl. *)
(* intros. *)
(* contradiction. *)
(* Focus 3. *)
(* intros. *)
(* apply rpo_mul_remove_equiv_aux1 with a0 a0. *)
(* Focus 2. *)


(* Focus 2. *)
(* intros. *)
(* apply List_mul with _ _ nil nil. *)
(* simpl in H2. *)
...
(* apply list_permut.permut0_sym. apply equiv_equiv. *)

(* assert (H3:= list_permut.permut_nil). *)
...
Theorem permut0_cons_app: forall a l l1, permut0 equiv ((a :: l) ++ l1) (l ++ a :: l1).
Proof.
intros.
assert ((a :: l) ++ l1 = a :: (l ++ l1)).
generalize l.
induction l0.
simpl. reflexivity.
simpl.
reflexivity.
rewrite H.
rewrite <- list_permut.permut0_cons_inside.
apply list_permut.permut_refl.
intros.
 apply (Relation_Definitions.equiv_refl _ _ equiv_equiv).
apply equiv_equiv.
 apply (Relation_Definitions.equiv_refl _ _ equiv_equiv).
...
Theorem rpo_mul_delete_list: forall rpo_infos A B l l1 l2, rpo_mul rpo_infos.(bb) A B -> permut0 equiv A (l ++ l1) -> permut0 equiv B (l++l2) -> rpo_mul rpo_infos.(bb) l1 l2.
Proof.
induction l.
simpl. intros.
destruct H.
apply List_mul with a lg ls lc.
apply list_permut.permut0_sym in H0.
apply list_permut.permut0_trans with l'.
apply equiv_equiv.
assumption.
assumption.
apply equiv_equiv.
apply list_permut.permut0_sym in H1.
apply list_permut.permut0_trans with l.
apply equiv_equiv. assumption. assumption.
apply equiv_equiv.
assumption.

intros.

apply rpo_mul_remove_equiv with a a.
 apply (Relation_Definitions.equiv_refl _ _ equiv_equiv).
apply IHl.
trivial.
trivial.
assert (H2: permut0 equiv ((a::l) ++ l1) (l ++ a :: l1)).
apply  list_permut.list_permut0_app_app.
Qed.

Theorem rpo_mult_eval_rpo_mul_less_not_greater: forall rpo_infos n A
B,  (forall t t' :term, mem equiv t B -> mem equiv t' A -> size t +
size t' <= n) ->  rpo_mult_eval rpo_infos n A B =  Some Less_than ->
rpo_mul rpo_infos.(bb) B A -> False.
Proof.
intros rpo_infos n A B H1 H Rpo_mul.
unfold rpo_mult_eval in H.
assert (H':= remove_equiv_eval_list_is_sound).
case_eq (remove_equiv_eval_list (equiv_eval rpo_infos n) A B).
intros p H3.
...
destruct l0.
discriminate.
assert (H2':= H' (equiv_eval rpo_infos n) A B).
rewrite H3 in H2'.
destruct  H2' as (l', (H01, (H02, (H03, H4)))).
destruct Rpo_mul. clear H4 H3 H'.
unfold mult_eval in H.

(* Theorem rpo_mul_rpo_mult_eval_less: forall rpo_infos n A B,  (forall t t' :term, mem equiv t B -> mem equiv t' A -> size t + size t' <= n) -> rpo_mul rpo_infos.(bb) A B -> rpo_mult_eval rpo_infos n A B =  Some Less_than. *)
(* Proof. *)
(* intros rpo_infos n A B H1 H. *)
(* unfold rpo_mult_eval in H. *)
(* assert (H':= remove_equiv_eval_list_is_sound).  *)
(* case_eq (remove_equiv_eval_list (equiv_eval rpo_infos n) A B). *)
(* intros p H3. *)
(* assert (H2':= H' (equiv_eval rpo_infos n) A B); clear H'. *)
(* rewrite H3 in H2'. *)
(* destruct p. *)
(* destruct  H2' as [ll [H01 [H02 [H03 H4]]]]. *)
(* Focus 2. *)

(* intros. *)
(* assert (H2:= H' (equiv_eval rpo_infos n) A B); clear H'. *)
(* rewrite H0 in H2. *)
(* destruct H2 as [t1 [t2 [in_A [in_B H2]]]]. *)
(* assert (H1':= mult_eval_is_sound_weak_equiv). *)
(* assert (H1'':= H1' rpo_infos n A B); clear H1'. *)
(* unfold remove_equiv_eval_list in H0. *)

(* apply (@List_mul _ _ l0 nil (map (fun st : term * term => fst st) ll)) in H. *)
(* apply permut0_eq_equiv. *)
(* assumption. *)
(* apply permut0_trans with (t :: l0 ++ map (fun st : term * term => snd st) l). *)
(* apply equiv_equiv. *)
...
(* Lemma rpo_antireflexive:  forall n lg0 , *)
(*    (forall b : term, *)
(*     mem equiv b lg0 -> *)
(*     exists a' : term, mem equiv a' lg0 /\ rpo n b a') -> False. *)
(* Proof. *)
(* intros  n A. *)
(* generalize A. *)
(* induction A0. *)
...
(* destruct H. *)
(* exists a. *)
(* assert (H':=equiv_rpo_equiv_1). *)
(* assert (H1:= H' n t a). *)
(* assert ( forall s : term, rpo n s t <-> rpo n s a). *)
(* apply H1; trivial. *)
(* assert (H0':= H0 t). *)
(* split. left. *)
(* apply (Relation_Definitions.equiv_refl _ _ equiv_equiv). *)
(* assert (H9:= rpo_antirefl). *)
(* assert (H9':= H9 n t). *)
(* destruct H0'. *)
(* assert (rpo n t t = False). *)
(* simpl. *)
(* rewrite <- H0'. *)
(* rewrite H0'. *)
(* assumption. *)
...
intro K3; apply False_rec; apply K3.
