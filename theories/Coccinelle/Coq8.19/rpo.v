...
Lemma equal_zero : forall n m, n = m + n -> m = 0.
Proof.
 induction n; intros; simpl in H. rewrite H. trivial.
 apply IHn. rewrite Nat.add_comm in H. simpl in H. inversion H.  rewrite <- H1. rewrite Nat.add_comm in H1.
 trivial. 
Qed. 
...
absurd (list_size size l < list_size size l); auto with arith. apply equal_zero in s_eq_t. 
assert (1 <= size a). apply size_ge_one. rewrite s_eq_t in H. contradict H. auto with arith.  
do 2 rewrite size_unfold;
simpl; apply lt_n_S_stt; apply Nat.lt_le_trans with (1 + list_size size l);
auto with arith; 
apply plus_le_compat_r_stt; apply size_ge_one. 
Defined.
...
unfold o_size2, size2, lex;
generalize (beq_nat_ok (size s) (size s)); case (Nat.eqb (size s) (size s)); [intros _ | intro s_diff_s].
do 2 rewrite size_unfold; apply plus_lt_compat_l_stt; simpl.
assert (1 <= size a). apply size_ge_one. generalize (list_size size l). intros. generalize H.  generalize (size a). intros. clear H. apply PeanoNat.le_lt_n_Sm in H0.  apply PeanoNat.lt_S_n in H0. apply Nat.lt_add_pos_l. trivial.  
apply False_rec; apply s_diff_s; reflexivity.
Defined.
...
do 2 rewrite size_unfold.
simpl; apply le_n_S.
assert (H:= proj1 (Nat.add_le_mono_r  0 (size t1) (list_size size l1))).
apply H. 
...
apply Nat.le_trans with (list_size size (a1 :: k1)); [apply le_plus_r_stt | exact p_le_k].
...
refine (Nat.le_trans _ _ _ _ L); apply le_plus_r_stt.
...
generalize (beq_nat_ok (size s) (size s)); case (Nat.eqb (size s) (size s)); [intros _ | intro s_diff_s].
do 2 rewrite size_unfold; apply plus_lt_compat_l_stt; simpl.
assert (H:= (Nat.add_le_mono_r (S 0) (size a) (list_size size l))).
assert (H': 1 + list_size size l <= size a + list_size size l). rewrite <- H. apply (size_ge_one a).  apply PeanoNat.le_lt_n_Sm in H'. simpl in H'. apply PeanoNat.lt_S_n in H'. trivial.
apply False_rec; apply s_diff_s; reflexivity.
Defined.
...
[apply Nat.le_add_r | apply plus_le_compat_r_stt; apply size_ge_one].
(* induction step *)
...
simpl; exists (@nil (term * term)); simpl; intuition auto with *.
...
rewrite <- beq_nat_refl_stt; simpl.
...
left; apply sym_eq; apply beq_nat_true_stt; assumption.
...
apply False_rec; generalize (beq_nat_true_stt _ _ L); clear L; induction ll as [ | [u1 u2] ll].
discriminate.
...
left; apply beq_nat_true_stt; assumption.
...
apply plus_le_compat_r_stt; apply size_direct_subterm; trivial.
intros t2 t2_in_l2; apply IHn.
...
apply plus_le_compat_l_stt; apply size_direct_subterm; trivial.
intros t1 t2 t1_in_l1 t2_in_l2; apply IHn.
apply le_S_n; refine (Nat.le_trans _ _ _ _ St);
replace (S (size t1 + size t2)) with  (S (size t1) + size t2); trivial;
...
rewrite Nat.add_comm; apply plus_le_compat_l_stt.
apply size_direct_subterm; trivial.
...
simpl in T; absurd (@None comp = None); trivial.
destruct L as [L | [L1 L2]].
rewrite L; rewrite <- beq_nat_refl_stt; intro; discriminate.
...
rewrite Nat.add_comm in H'; generalize (H' (Sl _ _ (or_introl _ (refl_equal _)) 
...
(* 1/1 *)
apply False_rec; apply T'; apply refl_equal.
try  intuition; try (absurd (rpo rpo_infos.(bb) t1 t1);
[intro; apply (@rpo_antirefl rpo_infos.(bb) t1); trivial|
rewrite (equiv_rpo_equiv_1 _ R') in t1_lt_t2; trivial]).
try  intuition; try (absurd (rpo rpo_infos.(bb) t1 t1);
[intro; apply (@rpo_antirefl rpo_infos.(bb) t1); trivial|
