...
From Coq Require Import List Setoid Morphisms PeanoNat EqNat Arith.
Require Import RelExtras MultisetCore ListPermutation NatUtil
...
#[export] 
  ...
  Section MultisetLemmas2.
    ...
    Proof.
      ...
      (* l > u *)
      assert ((x/R)%msets < (x/D)%msets); [lia | idtac]. assert (H':= Nat.sub_0_le).
      assert ( forall n m : nat,  n <= m -> (n - m)%nat = 0). intros. apply H'. trivial.
      rewrite H2.
